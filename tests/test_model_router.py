...
def test_router_uses_another_runnable_installed_local_model_when_default_is_missing_runtime(
    tmp_path,
):
    ...
    choice = model_router.choose_model(
        ...
        is_runtime_available=lambda model: model.name == "spare",
    ...
    assert choice.backend == "llama.cpp"
    assert choice.path == spare_path
    assert choice.context_window == 4096
    assert choice.reason == "installed local"
    assert choice.provider is None
    assert choice.account_id is None


def test_router_uses_runnable_installed_local_model_when_no_default_exists(tmp_path):
...
def test_router_preserves_default_unavailable_reason_when_all_local_models_are_unusable(tmp_path):
    ...
    assert choice.backend == "builtin"
    assert choice.path is None
    assert choice.reason == "runtime missing for default"
