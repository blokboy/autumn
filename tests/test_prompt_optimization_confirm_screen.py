...
async def test_launch_starts_a_live_run_when_none_is_active(tmp_path):
    ...
    async with app.run_test(size=(100, 60)) as pilot:
        ...
        # This screen's job is just getting Launch to hand off into a live run --
        # actually executing a real optimization end-to-end is #48's runtime and
        # its own test suite. `_install_fake_eval_asset` writes only metadata,
        # no train/examples data, so the real runtime (wired in by #48) fails
        # fast on this fixture; that's a fine, clean way to observe the FAILED
        # transition without needing a real GEPA-capable fixture here.
        for _ in range(20):
